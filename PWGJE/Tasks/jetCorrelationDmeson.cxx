...
/// \note Merges jetCorrelationD0.cxx and jetCorrelationDstar.cxx into a shared
///       struct, per design discussion with Jamie (data-level table/DeltaM
///       handling) and Matt (MC-detector category classification).
///       DCategory / classifyDCandidate() uses distinct numeric ranges for
///       D0 (0-5) and D* (6-8), per Matt's confirmation, so a bare category
...
// Shared D-meson (D0 / D*) candidate table -- data level.
...
// MC-detector: shared category column D0 uses 0-5
// (Signal/Reflection/CorrBkg1-4); D* uses its own distinct range 6-8
// (DstarSignal/DstarReflection/DstarCorrBkg) so the value is unambiguous.
...
// MC-detector level: shared table, category column confirmed by Matt.
...
// MC-particle level: identical structure/usage between D0 and D* already,
...
struct JetCorrelationDmeson {
  ...
  enum DCategory : int {
    ...
    Signal = 0,
    Reflection = 1,
    CorrBkg1 = 2, // D0ToPiKPi0
    CorrBkg2 = 3, // D0ToPiPi
    CorrBkg3 = 4, // D0ToPiPiPi0
    CorrBkg4 = 5, // D0ToKK
    // D*
    DstarSignal = 6,
    DstarReflection = 7,
  ...
  void init(InitContext const&)
  {
    ...
    // Delta-m: only ever filled for D*, but harmless to always book
  ...
  // ----------------------------------------------------------------------
  // Shared MC-particle logic. Confirmed identical structure between species
  // -- no species-specific branching needed.
  // ----------------------------------------------------------------------
  template <typename T, typename U, typename V>
